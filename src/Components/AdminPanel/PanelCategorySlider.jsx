    ...
    return (
        <div className="flex gap-2 overflow-y-auto mt-3 mb-6 border-b border-custom-color31 pb-6">
            {categoryData && categoryData.map(item => (
                <div
                    key={item.id}
                    className={`cursor-pointer ${selectedCategory === item.attributes.name ? 'border-t-2 border-custom-color31 rounded-t-xl transition duration-700 ' : 'border-t-2 border-custom-color1 rounded-t-xl transition duration-700'}`}
